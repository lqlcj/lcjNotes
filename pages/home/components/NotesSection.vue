<template>
  <div class="notes-section">
    ...
    <div class="xhs-container" ref="containerRef">
      <!-- 使用可复用的页面标题组件 -->
      <PageHeader title="My Stories" subtitle="记录生活，学习笔记" />
        ...
        >
          ...
          >
            <div
              ...
              :style="{ paddingBottom: (1 / item.visualRatio) * 100 + '%' }"
            >
              <!-- 图片加载占位符 -->
      ...
      <div
        v-if="totalPages > 1 && !isLoading && !hasError"
        class="pagination fade-in-up"
      >
        <button
        ...
        >
          <span
            v-if="isChangingPage && currentPage > 1"
            class="btn-spinner"
          ></span>
        ...
        >
          <span
            v-if="isChangingPage && currentPage < totalPages"
            class="btn-spinner"
          ></span>
      ...
      <div
        v-if="allData.length === 0 && !isLoading && !hasError"
        class="empty-tip glass-card"
      >
        <p>📝 还没有文章</p>
        <p style="margin-top: 10px; font-size: 0.9rem; color: #999">
          去
          <a href="/me" style="color: #6c5ce7; text-decoration: none"
            >后台管理</a
          >
          创建你的第一篇文章吧！
        </p>
...
const currentPageData = computed(() => {
  ...
  return pageData.map((item, index) => {
    const visualRatio =
      item.aspectRatio || ratioPattern[index % ratioPattern.length];
...
const distributeItems = () => {
  ...
  items.forEach((item) => {
    // 找到最短的列
    const shortestColIndex = columnHeights.value.indexOf(
