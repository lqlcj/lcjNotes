<template>
  <div class="tools-page">
    <div class="tools-container">
      <div class="tools-box">
        <!-- 页面标题 -->
        <PageHeader title="Tools" subtitle="一些实验功能~~." />
            ...
            <div v-if="activeTab === 'bookmarks'" class="tab-panel">
              <div class="bookmarks-content">
                ...
                <div v-else class="bookmarks-waterfall">
                  ...
                  >
                    <div
                      v-for="bookmark in getColumnItems(colIndex - 1)"
                      :key="bookmark.id"
                      class="bookmark-card"
                      :class="{
                        'showing-confirm': confirmingBookmarkId === bookmark.id,
                      }"
                      :ref="(el) => setCardRef(el, bookmark.id)"
                    >
                      <!-- 正常显示内容 -->
                      <div
                        v-if="confirmingBookmarkId !== bookmark.id"
                      ...
                      <div v-else class="bookmark-confirm">
                        <div class="confirm-content">
                          ...
                          <p class="confirm-disclaimer">
                            ⚠️ 外站内容与本站无关，请注意安全
                          </p>
                          <div class="confirm-actions">
                            <button
                              @click.stop="confirmVisit(bookmark)"
                              class="confirm-btn"
                            >
                              确认
                            </button>
...
const distributeItems = () => {
  ...
  items.forEach((item) => {
    ...
    const estimatedHeight =
      120 + (item.description ? 40 : 0) + (item.url ? 20 : 0);
